...
public class Graph
{
	...
	// можно сделать MakeGraph через массив Tupl-ов
	public static Graph MakeGraphTuple(params (int v1, int v2)[] incidentNodes)
	{
		var graph = new Graph(incidentNodes
			.SelectMany(n => new[] { n.v1, n.v2 })
			.Max() + 1);
		foreach (var edge in incidentNodes)
			graph.Connect(edge.v1, edge.v2);
		return graph;
	}
	...
	/// </summary>
	/// <param name="from">int From</param>
	/// <param name="to">int To</param>
	/// <returns>вернёт Edge</returns>
	public Edge this[int from, int to]
	{
		get => Edges
				.Where(e => e.From.NodeNumber == from && e.To.NodeNumber == to)
				.Single();
				// Single() будет работать, только если среди Edge нет повторов
				// Если есть, то можно использовать First()
